...
import { logoutUser, fetchViews, createView, deleteView, fetchStats, fetchPlanUsage } from '../../utils/api';
...
export default function Sidebar({
  ...
  // Load plan usage for thread limit display
  useEffect(() => {
  ...
  return (
    <aside className={styles.root}>
      ...
      <div className={styles.header}>
        <div className={styles.headerTop}>
          <div className={styles.logo}>
            ...
            <span className={styles.logoText}>{user?.workspace_name || 'BrandDesk'}</span>
          </div>
            ...
            <div style={{ position: 'relative' }}>
              ...
              {showUserMenu && (
                <div className={styles.userMenu}>
                  ...
                  {(user?.role === 'admin' || user?.workspace_role === 'owner' || user?.workspace_role === 'admin') && (
      ...
      {/* Thread usage — trial/starter only */}
      {planUsage && planUsage.limits.threads_per_month && (
        <div className={styles.planUsageBar}>
          <div className={styles.planUsageText}>
            <span>{planUsage.usage.threads_per_month} / {planUsage.limits.threads_per_month} threads this month</span>
            <span className={styles.planUsagePlan}>{planUsage.plan}</span>
          </div>
          <div className={styles.planUsageTrack}>
            <div
              className={styles.planUsageFill}
              style={{
                width: `${Math.min(100, (planUsage.usage.threads_per_month / planUsage.limits.threads_per_month) * 100)}%`,
                background: planUsage.usage.threads_per_month >= planUsage.limits.threads_per_month ? '#dc2626' :
                  planUsage.usage.threads_per_month >= planUsage.limits.threads_per_month * 0.8 ? '#f59e0b' : 'var(--accent)',
              }}
            />
          </div>
        </div>
