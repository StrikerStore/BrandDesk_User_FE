...
export default function CustomerPanel({ threadId }) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setThread(t);
      const { data } = await fetchCustomer(t.customer_email, t.brand);
      ...
      <div className={styles.body}>
        ...
        <div className={styles.section}>
          ...
          <div className={styles.detailRows}>
            ...
            {customer?.shopify_orders_count > 0 && (
              <div className={styles.detailRow}>
                <span className={styles.detailLabel}>Orders</span>
                <span className={styles.detailVal}>{customer.shopify_orders_count} orders · ₹{Number(customer.shopify_total_spent).toLocaleString('en-IN')}</span>
              </div>
            )}
            {!customer?.phone && !thread?.customer_phone && (
