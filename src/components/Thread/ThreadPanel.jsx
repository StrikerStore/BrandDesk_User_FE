...
export default function ThreadPanel({ threadId, brands, onThreadUpdate }) {
  ...
  // Helper: set text in both state and editor
  ...
  const handleSend = async () => {
    ...
    const ok = await reply({
      body: replyText,
      bodyHtml: bodyHtml !== replyText ? bodyHtml : undefined,
      isNote,
  ...
  const handleUseTemplate = async (tpl) => {
    ...
    setEditorText(resolveTemplate(tpl.body, vars));
    setShowTemplates(false);
    ...
    <div className={styles.root}>
      ...
      <div className={styles.replyArea}>
        ...
        <div className={styles.formatToolbar}>
          <button className={styles.formatBtn} onClick={() => execFormat('bold')} title="Bold (Ctrl+B)"><b>B</b></button>
          <button className={styles.formatBtn} onClick={() => execFormat('italic')} title="Italic (Ctrl+I)"><i>I</i></button>
          <button className={styles.formatBtn} onClick={() => execFormat('underline')} title="Underline (Ctrl+U)"><u>U</u></button>
          <span className={styles.formatDivider} />
          <button className={styles.formatBtn} onClick={handleInsertLink} title="Insert link">
            <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M10 13a5 5 0 007.54.54l3-3a5 5 0 00-7.07-7.07l-1.72 1.71"/><path d="M14 11a5 5 0 00-7.54-.54l-3 3a5 5 0 007.07 7.07l1.71-1.71"/></svg>
          </button>
          <button className={styles.formatBtn} onClick={() => execFormat('insertUnorderedList')} title="Bullet list">
          ...
          <button className={styles.formatBtn} onClick={() => execFormat('insertOrderedList')} title="Numbered list">
            <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><line x1="10" y1="6" x2="21" y2="6"/><line x1="10" y1="12" x2="21" y2="12"/><line x1="10" y1="18" x2="21" y2="18"/><text x="3" y="8" fontSize="8" fill="currentColor" fontFamily="sans-serif">1</text><text x="3" y="14" fontSize="8" fill="currentColor" fontFamily="sans-serif">2</text><text x="3" y="20" fontSize="8" fill="currentColor" fontFamily="sans-serif">3</text></svg>
          </button>
          <span className={styles.formatDivider} />
          <button className={styles.formatBtn} onClick={() => fileInputRef.current?.click()} title="Attach files">
        ...
        <div
          ref={editorRef}
          className={`${styles.editor} ${isNote ? styles.editorNote : ''}`}
          contentEditable
            ...
            {attachments.map((file, i) => (
              <div key={i} className={styles.attachmentChip}>
                {file.type.startsWith('image/') ? (
                  <img src={URL.createObjectURL(file)} alt={file.name} className={styles.attachmentThumb} />
                ) : (
              ...
              <>
                ...
                {aiOriginal && (
                  <button
                    ...
                    onClick={() => { setEditorText(aiOriginal); setAiOriginal(null); }}
