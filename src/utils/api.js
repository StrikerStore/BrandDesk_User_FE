...
// If backend silently upgraded an old token, persist the new one.
api.interceptors.response.use(res => {
  const refreshed = res.headers['x-refreshed-token'];
  if (refreshed) localStorage.setItem('bd_token', refreshed);
  return res;
}, err => {
...
export const sendReply = (gmailId, data) => {
  if (data instanceof FormData) {
    return api.post(`/api/threads/${gmailId}/reply`, data, { headers: { 'Content-Type': 'multipart/form-data' } });
  }
  return api.post(`/api/threads/${gmailId}/reply`, data);
};
...
// Customers
export const fetchCustomer       = (email, brand) => api.get(`/api/customers/${encodeURIComponent(email)}`, { params: { brand } });
...
export const disconnectShopify = (id)   => api.post(`/api/brands/${id}/shopify-disconnect`);
export const requestNewBrand    = (data)               => api.post('/api/brands/request', data);
...
export const markBrandSynced    = (brandId)             => api.post(`/api/brands/${brandId}/mark-synced`);
...
// Orders (external DB or Shopify)
export const fetchOrder         = (orderId, brand) => api.get(`/api/orders/${encodeURIComponent(orderId)}`, { params: { brand } });
export const fetchOrdersByEmail = (email, brand)   => api.get(`/api/orders/customer/${encodeURIComponent(email)}`, { params: { brand } });
...
export const fetchMyWorkspaces   = ()       => api.get('/api/workspaces/mine');
export const selectWorkspace     = (data)   => api.post('/api/users/select-workspace', data);
export const updateWorkspace     = (id, d)  => api.patch(`/api/workspaces/${id}`, d);
export const fetchWorkspaceMembers = (id)   => api.get(`/api/workspaces/${id}/members`);
export const logout              = ()       => { localStorage.removeItem('bd_token'); return api.post('/api/users/logout'); };

// Subscriptions / Billing
export const initiateSubscription = (data) => api.post('/api/subscriptions/initiate', data);
export const fetchSubscription    = ()     => api.get('/api/subscriptions/current');
...
export const createSupportTicket = (data)     => api.post('/api/support/tickets', data);
export const fetchSupportTicket  = (id)       => api.get(`/api/support/tickets/${id}`);
export const replySupportTicket  = (id, data) => api.post(`/api/support/tickets/${id}/reply`, data);
